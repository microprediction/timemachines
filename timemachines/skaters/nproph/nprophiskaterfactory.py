...
# Wraps the core nprophet prediction capability into a plain vanilla prediction function
# Unlike most skaters, this "integrated" nprophet skater (iskater) isn't computed by calling the skater
...
def nprophet_iskater_factory(y: [[float]], k: int, a: List = None, t: List = None, e=None,
    ...
    :param n_max:       Maximum number of observations to use, should you wish to prevent nprophet from slowing down
    ...
    :returns: x         k-vector of predictions
              ...
              forecast  full forecast dataframe, familiar to users of fbnprophet
    """
    if a:
    ...
    #   - It seems consistent to use nprophet to predict these forward,
...
def nprophet_fit_and_predict_with_time(y: [float], k: int, t: [float], model_params: dict = None) -> Tuple[
    List, List, Any, Any]:
...
def nprophet_fit_and_predict_with_time_and_advance_time(y: [float], k: int, t: [float], model_params: dict = None) -> \
...
def nprophet_fit_and_predict_with_advance_vars(y: [float], k: int, t: [float], a: [[float]],
...
def nprophet_fit_and_predict_with_exog_and_advance_vars_no_t(y: [[float]], k: int, freq: str, a: [[float]],
                                                            model_params: dict = None) -> Tuple[List, List, Any, Any]:
    """ Simpler wrapper for testing - univariate w/ advance vars w/ supplied times and future times  """
    assert len(a) == len(y) + k
    assert isinstance(y[0], List)
