"""The additive study: do the forecasts add value ON TOP of raw lags?
...
def _blr(keys):
    """river-native learner: their own BayesianLinearRegression, the
    suitable linear model the library already ships."""
    from river import linear_model, preprocessing

    class _M:
        def __init__(self):
            self.m = preprocessing.TargetStandardScaler(
                regressor=preprocessing.StandardScaler()
                | linear_model.BayesianLinearRegression())

        def predict_one(self, x):
            p = self.m.predict_one(x)
            return p if p is not None else 0.0

        def learn_one(self, x, y):
            self.m.learn_one(x, y)

    return _M()

CONDS = ("body", "base1", "b1_muy", "b1_mux", "b1_muxy", "b1_full",
         "ladder", "lad_muxy")
_EPS = 1e-12
...
def run_one(job):
    ...
    lad_keys = ["x"] + [f"x_{l}" for l in range(1, 5)] \
        + [f"y_{l}" for l in range(1, 9)]
    models = {
        "base1": _blr(["x", "y_1"]),
        "b1_muy": _blr(["x", "y_1", "mu_y"]),
        "b1_mux": _blr(["x", "y_1", "mu_x"]),
        "b1_muxy": _blr(["x", "y_1", "mu_x", "mu_y"]),
        "b1_full": _blr(["x", "y_1", "mu_x", "mu_y", "z_x", "z_y"]),
        "ladder": _blr(lad_keys),
        "lad_muxy": _blr(lad_keys + ["mu_x", "mu_y"]),
    }
    roll = _EwmaRoll()
    ew_zy = 0.0
    ...
    for t in range(n):
        ...
        aug = {"mu_x": mu_x, "mu_y": mu_y, "z_x": z_x, "z_y": zy}
        feats = {
            "base1": base1,
            "b1_muy": {**base1, "mu_y": mu_y},
            "b1_mux": {**base1, "mu_x": mu_x},
            "b1_muxy": {**base1, "mu_x": mu_x, "mu_y": mu_y},
            "b1_full": {**base1, **aug},
            "ladder": lad,
            "lad_muxy": {**lad, "mu_x": mu_x, "mu_y": mu_y},
        }
        for c in CONDS:
...
def summarize(results):
    ...
    print(f"\n=== additive study, BLR (n={len(results)}) ===")
    for variant in ("asis", "spikes"):
        rows = [r for r in results if r["variant"] == variant]
        if not rows:
            continue
        print(f"--- {variant} ({len(rows)} pairs) ---")
        for aug, base in (("b1_muy", "base1"), ("b1_mux", "base1"),
                          ("b1_muxy", "base1"), ("b1_full", "base1"),
                          ("lad_muxy", "ladder")):
            wins = sum(1 for r in rows if r[aug] < r[base])
            ratios = sorted(r[aug] / max(r[base], 1e-15) for r in rows)
            med = ratios[len(ratios) // 2]
            print(f"  {aug:9s} vs {base:6s}: wins {wins}/{len(rows)}, "
                  f"median ratio {med:.4f}")
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default=os.path.join(_HERE, "additive_results.jsonl"))
    args = ap.parse_args()
