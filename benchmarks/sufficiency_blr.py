"""The sufficiency study, river-native: BayesianLinearRegression.
...
CONDS = ("body", "five", "sixraw", "ewma5", "base1", "ladder")
_EPS = 1e-12
...
def run_one(job):
    ...
    models = {
        "five": _blr(["mu_x", "z_x", "mu_y", "z_y", "mu_y_prev"]),
        "sixraw": _blr(["x", "mu_x", "z_x", "mu_y", "z_y", "mu_y_prev"]),
        "ewma5": _blr(["mu_x", "z_x", "mu_y", "z_y", "mu_y_prev"]),
        "base1": _blr(["x", "y_1"]),
        "ladder": _blr(["x"] + [f"x_{l}" for l in range(1, 5)]
    ...
    for t in range(n):
        ...
        sixraw = dict(five); sixraw["x"] = xs[t]
        feats = {"five": five, "sixraw": sixraw, "ewma5": ew5,
                 "base1": base1, "ladder": lad}
        for c in CONDS:
            p = mu_y if c == "body" else models[c].predict_one(feats[c])
            if t >= burn:
                acc[c]["ae"] += abs(p - ys[t])
                acc[c]["n"] += 1
        for c in ("five", "sixraw", "ewma5", "base1", "ladder"):
...
def summarize(results):
    ...
    print(f"\n=== sufficiency, river BayesianLinearRegression (n={len(results)}) ===")
    for variant in ("asis", "spikes"):
        ...
        w6 = sum(1 for r, b in zip(rows, best) if r["sixraw"] <= 1.05 * b)
        ...
        print(f"  {variant} ({len(rows)} pairs): median five/best-raw {med:.3f}; "
              ...
              f"sixraw sufficient {w6}/{len(rows)}; best-raw beats body on {rb}")
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default=os.path.join(_HERE, "sufficiency_blr_results.jsonl"))
    args = ap.parse_args()
